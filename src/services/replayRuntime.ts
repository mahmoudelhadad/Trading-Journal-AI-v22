...
import { type ReplaySnapshot, type ReplayTimeframe } from '@apptypes/replay.js';
...
export function createReplayRuntime(deps: ReplayRuntimeDependencies): ReplayRuntime {
  ...
  let state: ReplaySnapshot = {
    ...
    loading: false, importing: false, error: null,
  };
  ...
  async function startReestablish(targetUtcMs?: number, targetSeries: HistoricalSeriesIdentity = state.series): Promise<void> {
    if (!attached) { pendingReestablish = true; return; }
    resumeAfterVisibility = false;
    pendingReestablish = false;
    const epoch = attachEpoch;
    const token = beginAuthoritativeOperation(authority);
    if (token === null) { pendingReestablish = true; return; }
    ...
    if (aborted !== null) { abortReestablish(aborted); return; }
    let availability;
  ...
  return {
    ...
    setTimeframe(timeframe) { publish({ timeframe }); },
    setSpeed(speed) { if (!isReplaySpeed(speed)) return false; publish({ speed }); reanchor(); return true; },
    play() { if (authority.barrierDepth > 0 || state.loading || state.error !== null) return; resumeAfterVisibility = false; publish({ playState: 'playing' }); reanchor(); },
    pause() { resumeAfterVisibility = false; publish({ playState: 'paused' }); reanchor(); },
    async goTo(utcMs) { if (!Number.isSafeInteger(utcMs)) return; await startReestablish(utcMs); },
    async stepForward() {
      resumeAfterVisibility = false;
